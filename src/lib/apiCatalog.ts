...
export const API_CATALOG: ApiEndpointDefinition[] = [
  ...
  "pagination": { "page": 1, "limit": 50, "total": 1, "pages": 1 }
    ...
    tables: ['agent_logs'],
    notes: ['This endpoint is intentionally excluded from agentLogger recursion.', 'Useful for operational audits and incident review.'],
  },
  {
    id: 'logs-stats',
    tag: 'Observability',
  ...
  },
  {
    ...
    description: 'Accepts multipart/form-data with one file plus optional folder path. Supabase uploads require a bearer token; R2 uploads do not.',
    auth: 'bearer-optional',
    mountsAt: '/api/media',
    sourceFile: 'api/routes/media.ts',
    logging: 'agentLogger',
    parameters: [
      ...
      { name: 'Authorization', in: 'header', required: false, type: 'Bearer token', description: 'Required when the active provider is Supabase.' },
    ],
    responseExamples: [
      {
        status: 200,
  ...
  "path": "product-images/hero.png"
      ...
      {
        status: 401,
        description: 'Supabase upload without token.',
        example: `{
  "error": "Authentication required for media uploads"
}`,
      },
    ],
  ...
  },
  {
    ...
    description: 'Deletes one file by storage path. Supabase deletes require a bearer token; R2 deletes use the worker bucket binding.',
    auth: 'bearer-optional',
    mountsAt: '/api/media',
    sourceFile: 'api/routes/media.ts',
    logging: 'agentLogger',
  ...
  "error": "CF_API_TOKEN is not set. Run: npx wrangler secret put CF_API_TOKEN"
    ...
    notes: ['Protected by worker environment configuration rather than caller auth headers.', 'Never returns secret values.'],
  },
  {
